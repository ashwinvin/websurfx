...
RUN export ARCH=$(uname -m) \
  && upx --lzma --best /app/target/$ARCH-unknown-linux-musl/release/anvesh \
  && cp /app/target/$ARCH-unknown-linux-musl/release/anvesh /usr/local/bin/anvesh


FROM --platform=$BUILDPLATFORM scratch
COPY --from=builder /app/public/ /opt/anvesh/public/
VOLUME ["/etc/xdg/anvesh/"]
COPY --from=builder /usr/local/bin/anvesh /usr/local/bin/anvesh
CMD ["anvesh"]
